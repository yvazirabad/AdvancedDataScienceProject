...
def buildclimatedict(path):
    coreweatherstationfile = r'C:\Users\ivazirabad\AdvancedDataScience\YearlyClimate\weather_station_pos\totals\coreweatherstation2.txt'
    cws = set()
    with open(coreweatherstationfile) as infile:
        infile.readline()
        for line in infile:
            splitline = line.rstrip().split('\t')
            cws.add('_'.join(splitline[1:]))
    ...
    for k, v in climatedict.items():
        if k not in cws or len(v) < 360:
            removed.add(k)
    for k in removed:
        del climatedict[k]

    print(year, c, len(climatedict), sep='\t')
    return climatedict
